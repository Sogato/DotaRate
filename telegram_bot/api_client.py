...
def _get_json(path: str, read_timeout: Optional[float] = READ_TIMEOUT) -> Optional[Any]:
    ...
    Тип результата зависит от метода: список матчей приходит как list, карточка матча и сводка прохода как dict.
    Параметр read_timeout позволяет долгим методам ждать ответ дольше обычного.
    """
    url = _url(path)
    try:
...
# ────────────────────────────────────────────────────────────────────────────
# 4. Списки за период и по лиге (для статистики)
# ────────────────────────────────────────────────────────────────────────────
