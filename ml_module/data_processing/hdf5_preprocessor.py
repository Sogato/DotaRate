...
- Воспроизводимое случайное разделение данных на тестовую и основную выборки
...
class HDF5Preprocessor:
    ...
    4. Случайное разделение ID на основную и тестовую выборки
    5. Потоковая загрузка данных матчей по чанкам
    6. Преобразование hero_id в плотные индексы для эффективного хранения
    7. Пакетная запись в HDF5 с оптимизированной структурой датасетов
    8. Валидация созданных файлов с детальным анализом содержимого

    Алгоритм разделения данных:
    - Получение всех валидных ID матчей из базы данных
    - При заданном MAX_TOTAL_MATCHES ограничение общего пула матчей до указанного размера
    - Случайный выбор TEST_DATASET_SIZE ID для тестовой выборки фиксированным генератором (RANDOM_SEED)
    ...
    def __init__(self, base_output_path: str, database_url: str = DATASET_DATABASE_URL):
        ...
        Raises:
            ValueError: Если MAX_TOTAL_MATCHES задан и меньше TEST_DATASET_SIZE,
                либо при некорректном формате database_url
    ...
    def _get_match_ids(self) -> Tuple[List[int], List[int]]:
        ...
        исключенных героев. При заданном MAX_TOTAL_MATCHES общий пул матчей
        воспроизводимо ограничивается до указанного размера. Затем полученные ID
        разделяются на две выборки для создания независимых датасетов.
        ...
        Note:
            ...
            нежелательными героями. Если задан MAX_TOTAL_MATCHES, из всех валидных
            матчей берётся случайная подвыборка указанного размера (а не первые N по
            match_id), чтобы избежать смещения в сторону старых матчей. Размер
            тестовой выборки ограничен константой TEST_DATASET_SIZE, остальные матчи
            попадают в основную выборку.

            Все случайные операции фиксируются сидом RANDOM_SEED: при неизменном
            наборе матчей один и тот же сид даёт идентичные выборки.
        """
        ...
        # Если задан лимит и доступных матчей больше — берём воспроизводимую случайную подвыборку
        if MAX_TOTAL_MATCHES is not None and len(match_ids) > MAX_TOTAL_MATCHES:
            limit_rng = np.random.default_rng(RANDOM_SEED)
            selected = limit_rng.choice(len(match_ids), size=MAX_TOTAL_MATCHES, replace=False)
            # Восстанавливаем порядок по match_id для консистентности
            match_ids = [match_ids[index] for index in sorted(selected)]

        # === АЛГОРИТМ СЛУЧАЙНОГО РАЗДЕЛЕНИЯ НА ВЫБОРКИ ===
        total_matches = len(match_ids)
        test_size = min(TEST_DATASET_SIZE, total_matches)

        # Случайный выбор индексов для тестовой выборки. Отдельный генератор с
        # фиксированным сидом делает разбиение воспроизводимым и не затрагивает
        # глобальное состояние numpy (в отличие от np.random.choice).
        rng = np.random.default_rng(RANDOM_SEED)
        test_indices = set(rng.choice(total_matches, size=test_size, replace=False))
    ...
    def create_hdf5(self):
        ...
        1. Получение, опциональное ограничение и случайное разделение ID матчей
        ...
        Алгоритм обработки:
        - При заданном MAX_TOTAL_MATCHES общий пул матчей ограничивается лимитом
        ...
        # Текстовое представление лимита для логов
