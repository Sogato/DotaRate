...
Длительность измеряется в секундах и имеет большой масштаб, неудобный для оптимизатора,
поэтому обучение ведётся в нормированном пространстве: цель стандартизуется (вычитается
среднее, делится на стандартное отклонение), и модель учится предсказывать нормированное
значение. Среднее и стандартное отклонение считаются один раз по всему обучающему набору
(общие для всех фолдов); это вносит незначительную утечку валидационных фолдов в
статистику нормализации.

Чтобы сохранённая модель была самодостаточной, разворот предсказания обратно в секунды
встроен в саму модель: перед сохранением к обученной сети добавляется фиксированный
(необучаемый) слой денормализации, выполняющий преобразование pred_сек = pred_норм * std + mean.
В результате сохранённый .keras файл сразу предсказывает длительность в секундах и не
требует никаких внешних файлов с параметрами нормализации. Обучение и оценка метрик при
этом по-прежнему выполняются в нормированном пространстве — слой денормализации в обучении
не участвует и на сходимость не влияет.
...
class ModelTrainerTimeV1:
    ...
    def train(self, data_file_path: str, base_model_path: str) -> None:
        ...
        6. Сохранение каждой модели с суффиксом _fold_N; перед сохранением к модели добавляется слой денормализации
        ...
        try:
            ...
            # Стандартизация цели: считаем mean/std по всему обучающему набору
    ...
    def _fit_target_normalization(self, labels_all: np.ndarray) -> np.ndarray:
        ...
        и сохраняются в атрибутах: они нужны для обучения в нормированном пространстве
        и для встраивания слоя денормализации при сохранении модели. Нулевое отклонение
    ...
    def _build_and_compile_model(self) -> Model:
        ...
        - Выход линейный и работает в стандартизованном пространстве цели.
        ...
        model = Model(
            ...
            outputs=output,
            name='dota2_time_predictor_train'
        )
    ...
    def _build_export_model(self, trained_model: Model) -> Model:
        ...
        К выходу обученной модели (в нормированном пространстве) добавляется один
        Dense(1) слой с замороженными весами, реализующий преобразование
        pred_сек = pred_норм * std + mean. За счёт этого сохранённая модель сразу
        предсказывает длительность в секундах и не требует внешних параметров
        нормализации.

        Веса слоя фиксированы и не обучаются: kernel = [[std]], bias = [mean].
        ...
        Returns:
            Model: Модель с выходом в секундах, готовая к сохранению
        """

        # Слой денормализации с фиксированными весами (не обучается)
        denorm_layer = Dense(1, activation='linear', trainable=False, name='denormalize')
        ...
        # Установка весов разворота: y = x * std + mean
        denorm_layer.set_weights([
            np.array([[self.target_std]], dtype=np.float32),  # kernel (1, 1)
            np.array([self.target_mean], dtype=np.float32)    # bias (1,)
        ])
