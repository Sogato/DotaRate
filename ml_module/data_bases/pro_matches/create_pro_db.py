"""
Скрипт создания базы данных для профессиональных матчей Dota 2.

Модуль отвечает за инициализацию структуры базы данных PRO матчей,
создавая все необходимые таблицы на основе определенных SQLAlchemy моделей.

Основное назначение:
- Первичная инициализация БД при развертывании проекта
...
from ml_module.data_bases.pro_matches.models import Base
...
def create_database() -> None:
    """
    Создает базу данных PRO матчей и все таблицы согласно определенным моделям.
    ...
    """
    print_section_header("СОЗДАНИЕ БД ПРОФЕССИОНАЛЬНЫХ МАТЧЕЙ", "🏆", color=Colors.BRIGHT_BLUE)
    ...
    try:
        ...
        print_status_message("База данных профессиональных матчей создана успешно!", "success", "✅")
...
def main() -> None:
    """
    Главная функция для создания БД PRO матчей.
    """
    create_database()
