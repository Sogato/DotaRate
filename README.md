...
Система состоит из трёх основных модулей, которые взаимодействуют для обеспечения полного цикла прогнозирования, и общего слоя с переиспользуемым кодом, на который они опираются.
...
- Тестирует обученные модели на отложенной выборке и профессиональных матчах
...
- Генерирует прогнозы на активные матчи через локальные копии моделей из ML-модуля
...
> 📄 **Документация**: [`docs/shared.md`](docs/shared.md)

Не отдельный модуль пайплайна, а фундамент, на который опираются остальные. Переиспользуемый код, не принадлежащий ни одному модулю в отдельности, вынесен в общие пакеты на верхнем уровне репозитория:
- Пакет `dota_core` — доменные константы и сущности Dota 2: общий config, `HeroMapper` и справочная БД героев (модель `Hero`) как общий источник истины об именах и метаданных героев
- Даёт модулям единый источник истины и не допускает дублирования — в частности, удерживает маппинг героев консистентным между обучением и инференсом
